...
The weights assigned to each algorithm for each outcome are shown in Table \@ref{fig:super_learner_weight_table}. Cross-validated risks for each outcome are shown in the following Section. 
...
The cross-validated performance of super learner (in terms of $R^2$ for continuous outcomes and AUC for dichotomous outcomes) relative to the discrete super learner and candidate algorithms are shown for IC-50 in Figure \@ref(fig:plotic50), for IC-80 in Figure \@ref(fig:plotic80), for IIP in Figure \@ref(fig:plotiip), and for the two dichotomous endpoints in Figure \@ref(fig:plotdichot1) and \@ref(fig:dichot2). 
...
class(cvfit_ic80) <- "myCV.SuperLearner"
plot(cvfit_ic80, main_title = "IC-50", xlim1 = -0.05, xlim2 = 0.8, text_size = 9,
     method = "method.CC_LS")
```
```{r plotiip, fig.cap = "Cross-validated R-squared for estimated IIP"}
class(cvfit_iip) <- "myCV.SuperLearner"
plot(cvfit_iip, main_title = "IC-50", xlim1 = -0.05, xlim2 = 0.8, text_size = 9,
     method = "method.CC_LS")
```

```{r plotdichot1, fig.cap = "Cross-validated AUC for predicting sensitivity based on estimated IC-50"}
class(cvfit_dichotomous.1) <- "myCV.SuperLearner"
