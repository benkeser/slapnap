...
#' @importFrom h2o as.h2o h2o.grid h2o.getGrid h2o.removeAll h2o.predict
#' @export
...

{
    ...
    grid <- h2o.getGrid(gbm.model@grid_id,
    ...
    gbm.final.model <- h2o.gbm(training_frame = dat.hex,
                                    y = "Y",
                                    distribution = h2o_dist,
                                    stopping_metric = toupper(h2o_metric),
                                    stopping_rounds = 3,
    ...
    # Get predictions
    pred.raw <- h2o.predict(object = gbm.final.model,
                              newdata = newX.hex)
...
predict.SL.h2oboost <- function(object, newdata, ...)
{
    ...
    # Get predictions
    pred.raw <- h2o.predict(object = object$object,
                              newdata = newdata.hex)
    ...
    nthread = 1, verbose = 0, save_period = NULL, ...)
{
    ...
    xgmat = xgb.DMatrix(data = X, label = Y, weight = obsWeights)
    if (family$family == "gaussian") {
        model = xgboost(data = xgmat, objective = "reg:squarederror",
            nrounds = ntrees, max_depth = max_depth, min_child_weight = minobspernode,
            eta = shrinkage, verbose = verbose, nthread = nthread,
            params = params, save_period = save_period)
    }
    if (family$family == "binomial") {
        model = xgboost(data = xgmat, objective = "binary:logistic",
            nrounds = ntrees, max_depth = max_depth, min_child_weight = minobspernode, eval_metric = "logloss",
            eta = shrinkage, verbose = verbose, nthread = nthread,
            params = params, save_period = save_period)
    }
    if (family$family == "multinomial") {
        model = xgboost(data = xgmat, objective = "multi:softmax",
            nrounds = ntrees, max_depth = max_depth, min_child_weight = minobspernode,
...
#' @export
SL.ranger.imp <- function(
    Y, X, newX, family, obsWeights = rep(1, length(Y)), num.trees = 500, mtry = floor(sqrt(ncol(X))),
...
tmp_method.CC_LS <- function ()
{
    computeCoef = function(Z, Y, libraryNames, verbose, obsWeights,
        ...
        compute <- function(x, y, wt = rep(1, length(y))) {
            ...
            fit <- tryCatch({solve.QP(Dmat = D, dvec = d, Amat = A,
                bvec = bvec, meq = 1)
          }, error = function(e){
            out <- list()
            class(out) <- "error"
...
tmp_method.CC_nloglik <- function ()
{
    ...
    computeCoef = function(Z, Y, libraryNames, obsWeights, control,
        ...
        r <- tryCatch({nloptr(x0 = rep(1/ncol(modZ), ncol(modZ)),
...
#' @importFrom vimp extract_sampled_split_predictions get_cv_sl_folds
...
sl_one_outcome <- function(complete_dat, outcome_name,
  ...
  if (length(opts$learners) == 1 & opts$cvtune & opts$cvperf) {
    ...
    if(call_out){
        ...
        vimp_cf_folds <- get_cv_sl_folds(cv_fit$folds)
        cv_preds <- extract_sampled_split_predictions(cvsl_obj = cv_fit, sample_splitting = TRUE, sample_splitting_folds = ss_folds, full = full_fit)
