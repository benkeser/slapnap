...
#' Get system variables
#'
#' @param option A character string.
#' @param boolean A booloean.
#' @returns Required parameters as expected by program
#' @export
#' @examples
#' get_sys_var("nab", FALSE)
#' get_sys_var("cvtune", TRUE)
...
#'
#' @param options A character vector.
#' @param options_boolean A booloean vector.
#' @returns Named list of system variables
#' @export
#' @examples
#' get_global_options(c("nab", "cvtune"), c(FALSE, TRUE))
get_global_options <- function(options = c("nab", "outcomes", "learners", "cvtune", "cvperf", "nfolds", "combination_method", "binary_outcomes", "importance_grp", "importance_ind", "ind_importance_type", "report_name", "return", "sens_thresh", "multsens_nab", "same_subset", "var_thresh", "output", "slfits", "analysis"),
...
get_cv_outcomes_tables <- function(fit_list_out = NULL, run_sls = NULL, run_sls2 = NULL, opts){
    ...
    if(length(cont_idx) > 0){
        ...
        rsq_kab <- kable(rsqtab, col.names = c("CV-R$^2$", "Lower 95% CI", "Upper 95% CI"),
    ...
    if(length(dich_idx) > 0){
        ...
        auc_kab <- kable(auctab, col.names = c("CV-AUC", "Lower 95% CI", "Upper 95% CI"),
              digits = 3, row.names = TRUE,
...
#' @export summary.myCV.SuperLearner
