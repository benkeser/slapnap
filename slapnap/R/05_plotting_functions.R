...
make_logo_plot <- function(data,
  ...
  tmp <- suppressWarnings(as.numeric(word(names(X),2,2,sep = '\\.')))
  tmp[is.na(tmp)] <- 999999
  for (position in aa_positions){
    # position = aa_positions[1]
    X0 = X[ , tmp == position]
    names(X0) = word(names(X0),3,3,sep = '\\.')
  ...
  p1 = ggseqlogo( frequency_table_y1, method='prob', seq_type = "aa") + ylab(ifelse(outcome_name == "dichotomous.1", "Estimated resistant", "Multiply resistant") ) + theme(legend.position = 'none')
  p1$scales$scales[[1]] <- scale_x_continuous(breaks = 1:length(aa_positions),labels=aa_positions)
  p0 = ggseqlogo( frequency_table_y0, method='prob', seq_type = "aa") + ylab(ifelse(outcome_name == "dichotomous.1", "Estimated sensitive", "Multiply sensitive") )
  p0$scales$scales[[1]] <- scale_x_continuous(breaks = 1:length(aa_positions),labels=aa_positions)
  grid.arrange(p1, p0)
...
plot_cv_predictions <- function(cv_fit,
  ...
  cv_fold_palette <- brewer.pal(max(c(3,cv_folds)), "Set1")
  if(!resid_scale){
...
plot_roc_curves <- function(cv_fit, topRank = 1,
  ...
  roc.obj <- predict %>%
    group_by(algo) %>%
    nest() %>%
    mutate(pred.obj = map(data, ~ prediction(.x$pred, .x$Y)),
           perf.obj = map(pred.obj, ~ performance(.x, "tpr", "fpr")),
           roc.dat = map(perf.obj, ~ tibble(xval = .x@x.values[[1]],
                                                   yval = .x@y.values[[1]])))
  roc.obj %>%
    unnest(roc.dat) %>%
...
plot_predicted_prob_boxplots <- function(cv_fit, topRank = 1, opts){
  ...
  cv_fold_palette <- brewer.pal(max(c(cv_folds,3)), "Set1")
  predict %>%
    ...
    if (package == "ggplot2") {
      this_y_lab <- if(Rsquared){
        ...
        p <- ggplot(d, aes_string(x = "X",
            ...
            coord_flip() + ylab(this_y_lab) +
            xlab("Method") +
            ggtitle(main_title) +
            scale_y_continuous(limits=c(xlim1, xlim2), oob = squish) +
...
create_CVROC_plots = function(dataset, fit, SL.library, outcome, covarX, plotFile, ...){
  ...
  tbl_forPredProb_wide = tbl_forPredProb_wide %>% select(Sensitivity, V2, V3, V4, V5)
...
dataprep_outValidate <- function(fit=fit, outcome, covarX, outcome_type, ...){
  ...
  tab <- tab %>% mutate(screen.algo=gsub(".skinny|screen.", "", screen.algo)) %>%
    mutate(screen=sapply(strsplit(screen.algo, "_"), `[`, 1)) %>%
    mutate(algo=sapply(strsplit(screen.algo, "_"), `[`, 2)) %>%
...
dataprep <- function(fit=fit, ...){
  tab <- summary(fit, "learners")[[1]] %>% mutate(SL_wrap = as.character(SL_wrap))  %>%
    mutate(screen_algo=gsub(".skinny|screen.", "", SL_wrap)) %>%
    mutate(screen=sapply(strsplit(screen_algo, "_"), `[`, 1)) %>%
    mutate(algo=sapply(strsplit(screen_algo, "_"), `[`, 2)) %>%
...
forestplot_allmodels_continuousOutcome <- function(data, datasetName, plotFile, plotymin, plotymax, ...){
  ...
  p = p + geom_rect(aes(xmin = -Inf, xmax = 1.5, ymin = -Inf, ymax = Inf), fill = "gray43", alpha = 0.006) +
    ...
    theme(legend.position="", axis.title = element_text(size=18), axis.text.x = element_text(size=45), axis.text.y = element_text(size=45),
          panel.background = element_blank(),
          axis.title.x = element_text(size=55, margin = margin(t = 20, r = 0, b = 0, l = 0)),
          axis.title.y = element_blank(), axis.ticks = element_blank(),
          panel.grid.major = element_line("gray90"),
          plot.margin=unit(c(3,0.5,0.5,20),"cm")) +
    scale_x_discrete(labels=xticks)

  algos = as.data.frame(table(data$algo)) %>% filter(Freq!=0)
  algos = algos$Var1
...
# ---------------------------------------------------------------------------- #
#' @importFrom dplyr filter mutate rename full_join select
#' @export
getTOP10models <- function(tab, tab2, outcomeType, writeFile, ...){

  tab2 = as.data.table(tab2)  %>% filter(abs(cv_measure)<100, abs(ci_low)<100, abs(ci_high)<100) %>% mutate(Validated_R2num = cv_measure) %>%
    mutate(Validated_R2=format(round(Validated_R2num, 3), nsmall=3), ci_low=format(round(ci_low, 3),nsmall=3), ci_high=format(round(ci_high, 3),nsmall=3)) %>%
    mutate(Validated_R2 = paste0(Validated_R2, " (", ci_low, ", ", ci_high, ")")) %>% select(screen, algo, Validated_R2, Validated_R2num)

  tab = as.data.table(tab)  %>% filter(abs(cv_measure)<100, abs(ci_low)<100, abs(ci_high)<100) %>% mutate(CV_R2num = cv_measure) %>%
    mutate(CV_R2=format(round(CV_R2num, 3), nsmall=3), ci_low=format(round(ci_low, 3),nsmall=3), ci_high=format(round(ci_high, 3),nsmall=3)) %>%
    mutate(CV_R2 = paste0(CV_R2, " (", ci_low, ", ", ci_high, ")")) %>% select(screen, algo, CV_R2, CV_R2num) %>%
    full_join(tab2, by=c("screen", "algo")) %>% arrange(desc(CV_R2num))
  ...
  tab = tabTop10 %>% full_join(tab2Top10, by=c("screen", "algo", "CV_R2", "Validated_R2")) %>% arrange(desc(CV_R2num.x), desc(Validated_R2num.y)) %>%
    select(screen, algo, CV_R2, Validated_R2)
  ...
  if(outcomeType=="dichotomous")
    tab = tab %>% rename(CV_AUC = CV_R2, Validated_AUC = Validated_R2)
  write.csv(tab, file=writeFile)
}
