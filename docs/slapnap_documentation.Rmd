...
```{r gp120, fig.width = 7, echo = FALSE}
knitr::include_graphics("gp120.png")
```
```{block, fig.width = 5, type = "center"}
...
The date of last build of `slapnap` (and thus the last date data were downloaded from CATNAP) is `r format(Sys.time(), '%B %d, %Y')`.  
