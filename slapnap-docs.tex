...
programs running inside the container. To avoid possible naming
conflicts in the mounted directory, we recommend mounting an empty
...
\item
  ...
  and
\item
  sensitivity: the binary indicator that IC\(_{50}\) \(<\)
...
\item
  ...
  and
\item
  estimated sensitivity: the binary indicator that estimated IC\(_{50}\)
