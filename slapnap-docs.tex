...
\tightlist
\item
  descriptions of the requested outcomes (including how the outcomes are
  generated, if they are derived from IC\(_{50}\) or IC\(_{80}\));
\item
...
\item
  ...
  feature group and outcome (if \texttt{"marg"} is specified in
  \texttt{importance\_grp}); and
\item
  a table of ranked conditional biological prediction performance for
  each feature group and outcome (if \texttt{"cond"} is specified in
  \texttt{importance\_grp}).
\end{itemize}

By default, the results section for each requested outcome contains
descriptive statistics and references. The descriptive statistics
include summaries of the distribution of the outcome (raw and
log-transformed) for each bNAb for continuous outcomes and number
sensitive/resistant for binary outcomes. Based on the specific options
passed to \texttt{slapnap}, the following subsections may also be
present:
...
\item
  ...
  ensemble): figures showing cross-validated prediction performance (all
  outcomes), cross-validated receiver operating characteristic (ROC)
  curves (binary outcomes), and cross-validated predicted probabilities
  of resistance (binary outcomes); and
\item
...
groups are displayed in a section immediately preceeding the references.

\section{Data details}\label{sec:data}

The analysis dataset includes neutralization outcomes for the requested
bNAb(s) and AA sequence features for the gp160 protein. If a single bNAb
is requested, then the possible outcomes are:
...
\item
  ...
  IC\$\_\{50\} \textless{} \$ the sensitivity threshold
...
\tightlist
\item
  geographic confounders: binary indicator variables describing the
  geographic region of origin for the given pseudovirus;
\item
  subtype: binary indicator variables denoting the subtype for the given
  pseudovirus;
\item
...
For single antibodies and antibody combinations measured directly in the
CATNAP database, IC\(_{50}\) and IC\(_{80}\) are available. Based on the
measured IC\(_{50}\) a binary sensitive/resistant endpoint is created
(if \texttt{sens} is included in the \texttt{-e\ outcomes} option). A
pseudovirus is defined as \emph{resistant} (\texttt{sens\ =\ 1}) if its
measured IC\(_{50}\) is greater than the user-specified sensitivity
threshold (specified in the \texttt{-e\ sens\_thresh} option) and
\emph{sensitive} otherwise (\texttt{sens\ =\ 0}).
