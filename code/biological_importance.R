...
for (i in 1:length(outcome_names)) {
    ...
    if (run_sl_vimp_bools$run_vimp[i]) {
        ...
        vimp_plot_type_expr <- unlist(lapply(as.list(names(current_vimp_lst)), vimp_plot_type_expr))
        vimp_plot_titles <- unlist(lapply(vimp_type_expr, function(x) eval(bquote(expression(.(plot_title_expr[[1]])~.(x))))))
