...
check_signif <- function(x, vimp_threshold) any(unlist(lapply(x, function(z) any(z$p_value < vimp_threshold))))
...
for (i in 1:length(outcome_names)) {
    ...
    if (run_sl_vimp_bools2$run_vimp[i]) {
        ...
        ## set whether or not any were significant
        eval(parse(text = paste0(this_outcome_name, "_any_signif <- check_signif(", this_outcome_name, "_vimp_lst)")))
        eval(parse(text = paste0(this_outcome_name, "_any_signif_cv <- check_signif(", this_outcome_name, "_cv_vimp_lst)")))
...
if (opts$cvperf) {
    vimp_summary_tbl <- make_vimp_executive_summary_table(
        ...
        any_signif_all <- any(log10.pc.ic50_any_signif, log10.pc.ic80_any_signif, iip_any_signif, dichotomous.1_any_signif, dichotomous.2_any_signif)
} else {
    ...
    any_signif_all <- any(log10.pc.ic50_any_signif_cv, log10.pc.ic80_any_signif_cv, iip_any_signif_cv, dichotomous.1_any_signif_cv, dichotomous.2_any_signif_cv)
}
