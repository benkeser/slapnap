...
* `r ncomplete_features` sequences had complete feature information;

* `r ncomplete_ic50`, `r ncomplete_ic80`, and `r ncomplete_ic5080` sequences had complete data for `r ifelse(one_nab, "", est_fillin)`IC-50, `r ifelse(one_nab, "", est_fillin)`IC-80, and both `r ifelse(one_nab, "", est_fillin)`IC-50 and `r ifelse(one_nab, "", est_fillin)`IC-80, respectively, and were used in the analysis`r ifelse(any_dich, ";", ".")`
`r if(!("sens1" %in% opts$outcomes)) "<!--"`
* out of the sequences with complete data for `r ifelse(one_nab, "", est_fillin)`IC-50, `r sum(dat$dichotomous.1)` were estimated to be resistant to the `r ifelse(!one_nab, "combination of antibodies", "antibody")`, while `r ncomplete_ic50 - sum(dat$dichotomous.1)` were estimated to be sensitive`r ifelse(!one_nab, " (using estimated sensitivity)", "")``r check_sl_vimp_bin(opts, run_sl_vimp_bools, "sens1")``r ifelse("sens2" %in% opts$outcomes, ";", ".")`
`r if(!("sens1" %in% opts$outcomes)) "-->"`
`r if(!("sens2" %in% opts$outcomes)) "<!--"`
* out of the sequences with complete data for `r ifelse(one_nab, "", est_fillin)`IC-50, `r sum(dat$dichotomous.2)` were estimated to be resistant to the `r ifelse(!one_nab, "combination of antibodies", "antibody")`, while `r ncomplete_ic50 - sum(dat$dichotomous.2)` were estimated to be sensitive (using multiple sensitivity)`r check_sl_vimp_bin(opts, run_sl_vimp_bools, "sens2")`
`r if(!("sens2" %in% opts$outcomes)) "-->"`
...
## make the table
kable(vimp_summary_marg, col.names = c("Variable group", colnames(vimp_summary_marg)[-1]), caption = get_biological_importance_table_description(opts, cont_nms, bin_nms, num_obs_fulls, num_obs_reds, n_row_now, vimp_threshold, importance_type = "marginal"))
```
...
correct_description <- paste0(" Importance is measured via ", cont_nm_descr, ifelse(length(cont_nms) > 0 & length(bin_nms) > 0, " and ", ""), bin_nm_descr, ".")
## make the table
kable(vimp_summary_cond, col.names = c("Variable group", colnames(vimp_summary_cond)[-1]), caption = get_biological_importance_table_description(opts, cont_nms, bin_nms, num_obs_fulls, num_obs_reds, n_row_now, vimp_threshold, importance_type = "conditional"))
```
...
```{r, histic50, eval = ("ic50" %in% opts$outcomes) & !one_nab, fig.cap = paste0("Distribution of IC-50 values (n = ", ncomplete_ic50, ") for each neutralizing antibody."), fig.height=5}
...
kable(ic50_table, row.names = TRUE, digits = 3,
      caption = paste0("Summary statistics of IC-50 values for individual bnAbs (n = ", ncomplete_ic50, " observations with complete sequence data)."))
```
...
ic50_figcap <- if(length(opts$nab) == 1){
  paste0("Histogram of IC-50 for the selected broadly neutralizing antibody. Top row = original scale; bottom row = log10 scale (n = ", ncomplete_ic50, " observations with complete sequence data).")
}else{
  paste0("Histogram of estimated IC-50 for the combined neutralizing antibodies. Top row = original scale; bottom row = log10 scale (n = ", ncomplete_ic50, " observations with complete sequence data).")
}
```
...
kable(as.data.frame(weight_table), digits = 2,
             caption = paste0("Table of super learner weights for ", ifelse(length(opts$nab) == 1, "", "estimated"), " IC-50 (n = ", ncomplete_ic50, " observations with complete sequence data)."))
```
...
```{r plotic50, eval = ("ic50" %in% opts$outcomes) & (opts$cvperf), fig.cap = paste0("Cross-validated $R^2$ for ", ifelse(length(opts$nab) == 1, "", "estimated"), " IC-50 (", ncomplete_ic50, " observations with complete sequence data)")}
...
```{r plotic50predversusoutcomes, eval = ("ic50" %in% opts$outcomes) & (opts$cvperf), fig.cap = paste0("Cross-validated super learner predicted ", ifelse(length(opts$nab) == 1, "", "estimated"), " IC-50 plotted against actual IC-50 values (", ncomplete_ic50, " observations with complete sequence data) . Colors correspond to cross-validation folds.")}
...
```{r, histic80, eval = ("ic80" %in% opts$outcomes) & (!one_nab), fig.cap = paste0("Distribution of IC-80 values (n = ", ncomplete_ic80, ") for each neutralizing antibody."), fig.height=5}
...
kable(ic80_table, row.names = TRUE, digits = 3,
      caption = paste0("Summary statistics of IC-80 values for individual bnAbs (n = ", ncomplete_ic80, " observations with complete sequence data)."))
```
...
ic80_figcap <- if(length(opts$nab) == 1){
  paste0("Histogram of IC-80 for the selected broadly neutralizing antibody. Top row = original scale; bottom row = log10 scale (n = ", ncomplete_ic80, " observations with complete sequence data).")
}else{
  paste0("Histogram of estimated IC-80 for the combined neutralizing antibodies. Top row = original scale; bottom row = log10 scale (n = ", ncomplete_ic80, " observations with complete sequence data).")
}
```
...
# row.names(weight_table) <- this_library # will be defined in executive summary chunk
kable(as.data.frame(weight_table), digits = 2,
             caption = paste0("Table of super learner weights for ", ifelse(length(opts$nab) == 1, "", "estimated"), " IC-80 (n = ", ncomplete_ic80, " observations with complete sequence data)."))
```
...
```{r plotic80, eval = ("ic80" %in% opts$outcomes) & (opts$cvperf), fig.cap = paste0("Cross-validated $R^2$ for ", ifelse(length(opts$nab) == 1, "", "estimated "), "IC-80 (", ncomplete_ic80, " observations with complete sequence data)")}
...
Out of the `r ncomplete_ic50` sequences, `r sum(dat$dichotomous.2)` were estimated to be resistant to the `r ifelse(!one_nab, "combination of antibodies", "antibody")`, while `r ncomplete_ic50 - sum(dat$dichotomous.2)` were estimated to be sensitive.
