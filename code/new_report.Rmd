...
* out of the sequences with complete data, `r sum(dat$dichotomous.1)` were estimated to be resistant to the `r ifelse(length(opts$nab) > 1, "combination of antibodies", "antibody")`, while `r ncomplete - sum(dat$dichotomous.1)` were estimated to be sensitive (using estimated sensitivity)`r ifelse("sens2" %in% opts$outcomes, ";", ".")`
`r if(!("sens1" %in% opts$outcomes)) "-->"`
`r if(!("sens2" %in% opts$outcomes)) "<!--"`
* out of the sequences with complete data, `r sum(dat$dichotomous.2)` were estimated to be resistant to the `r ifelse(length(opts$nab) > 1, "combination of antibodies", "antibody")`, while `r ncomplete - sum(dat$dichotomous.2)` were estimated to be sensitive (using multiple sensitivity).
`r if(!("sens2" %in% opts$outcomes)) "-->"`
...
The predictive ability of the learner was assessed using cross-validation. `r if(any_cont & opts$cvperf) "The estimated cross-validated $R^2$ of the super learner for predicting continuous outcomes are shown in Table \\@ref(tab:rsquaredtable)."` `r if(any_dich & opts$cvperf) "The estimated cross-validated area under the receiver operating characteristic curve (AUC) of the super learner for predicting binary sensitivity measures are shown in Table \\@ref(tab:auctable)."`
...
We define the conditional biological importance of a subgroup of features for predicting an outcome as the difference in population predictiveness between the best possible prediction function based on all available features versus all features except those under consideration. In Table \@ref(tab:vimpgrpcond), we display the groups of variables and their ranked conditional biological variable importance for predicting `r ifelse(length(opts$outcomes) == 1, get_comma_sep_outcomes(opts), "each outcome.")` Stars next to ranks denote groups with p-value less than `r vimp_threshold` from a hypothesis test with null hypothesis of zero importance. For variable group definitions, please refer to Table \@ref(tab:vimpGroupTab).
...
# if saving figures was requested, save
if ("figures" %in% opts$return) {
 ggsave(filename = paste0("/home/output/ic50_slplot_", postfix, ".png"), plot = plot(fit_list_out$out$ic50, main_title = "IC-50", xlim1 = -0.05, xlim2 = 0.8,
...
# if save figs, save
if ("figures" %in% opts$return) {
    ggsave(filename = paste0("/home/output/ic50_cvpred_vs_outcomes_", postfix, ".png"), plot = plot_cv_predictions(cv_fit = fit_list_out$out$ic50, outcome_name = "IC-50", log_axis = FALSE), width = 20, height = 20, units = "cm")
}
```
`r if (!opts$cvperf | !("ic50" %in% opts$outcomes)) "-->"`
...
The biological variable importance of groups of features (defined in Table \@ref(tab:vimpGroupTab)) for predicting IC-50 is shown in Figure \@ref(fig:ic50groupvimp). Importance is defined using the difference in $R^2$ values.
...
```{r ic50groupvimp, eval = ("ic50" %in% opts$outcomes) & (("marg" %in% opts$importance_grp) | ("cond" %in% opts$importance_grp)), fig.cap = biological_importance_figure_caption(ncomplete, num_obs_full, num_obs_red, "ic50", grp = TRUE), fig.subcap = c("Marginal importance", "Conditional importance")}
...
We show the biological variable importance of individual features in predicting IC-50 in Figure \@ref(fig:ic50indivimp). Importance is defined using the difference in $R^2$ values.
...
```{r ic50indivimp, eval = ("ic50" %in% opts$outcomes) & (("marg" %in% opts$importance_ind) | ("cond" %in% opts$importance_ind)), fig.cap = biological_importance_figure_caption(ncomplete, num_obs_full, num_obs_red, "ic50", grp = FALSE), fig.subcap = c("Marginal importance", "Conditional importance")}
...
if ("figures" %in% opts$return) {
    ggsave(filename = paste0("/home/output/ic50_biol_import_ind_", postfix, ".png"), plot = grid.arrange(grobs = list(ic50_ind_vimp_grob_lst), ncol = length(opts$importance_ind), top = "Individual Biological Variable Importance"), width = 20, height = 20, units = "cm")
}
```
...
plot(fit_list_out$out$ic80, main_title = "IC-80", xlim1 = -0.05, xlim2 = 0.8,
     text_size = 9, method = "method.CC_LS", opts = opts)
if ("figures" %in% opts$return) {
ggsave(filename = paste0("/home/output/ic80_slplot_", postfix, ".png"), plot = plot(fit_list_out$out$ic80, main_title = "IC-80", xlim1 = -0.05, xlim2 = 0.8,
     text_size = 9, method = "method.CC_LS", opts = opts), width = 20, height = 20, units = "cm")
}
```
...
# if save figs, save
if ("figures" %in% opts$return) {
    ggsave(filename = paste0("/home/output/ic80_cvpred_vs_outcomes_", postfix, ".png"), plot = plot_cv_predictions(cv_fit = fit_list_out$out$ic80, outcome_name = "IC-80", log_axis = FALSE), width = 20, height = 20, units = "cm")
}
```
...
We show the biological variable importance of groups of features (defined in Table \@ref(tab:vimpGroupTab)) in predicting IC-80 in Figure \@ref(fig:ic80groupvimp). Importance is defined using the difference in $R^2$ values.
`r get_biological_importance_plot_description(opts, grp = TRUE)` `r (1-vimp_threshold)*100`\% confidence intervals and stars denoting p-values less than `r vimp_threshold` are displayed in blue.


```{r ic80groupvimp, eval = ("ic80" %in% opts$outcomes) & (("marg" %in% opts$importance_grp) | ("cond" %in% opts$importance_grp)), fig.cap = biological_importance_figure_caption(ncomplete, num_obs_full, num_obs_red, "ic80", grp = TRUE), fig.subcap = c("Marginal importance", "Conditional importance")}
if (("marg" %in% opts$importance_grp) & ("cond" %in% opts$importance_grp)) {
...
We show the biological variable importance of individual features in predicting IC-80 in Figure \@ref(fig:ic80indivimp). Importance is defined using the difference in $R^2$ values.
`r get_biological_importance_plot_description(opts, grp = FALSE)` `r (1-vimp_threshold)*100`\% confidence intervals and stars denoting p-values less than `r vimp_threshold` are displayed in blue.


```{r ic80indivimp, eval = ("ic80" %in% opts$outcomes) & (("marg" %in% opts$importance_ind) | ("cond" %in% opts$importance_ind)), fig.cap = biological_importance_figure_caption(ncomplete, num_obs_full, num_obs_red, "ic80", grp = FALSE), fig.subcap = c("Marginal importance", "Conditional importance")}
if (("marg" %in% opts$importance_ind) & ("cond" %in% opts$importance_ind)) {
...
if ("figures" %in% opts$return) {
    ggsave(filename = paste0("/home/output/ic80_biol_import_ind_", postfix, ".png"), plot = grid.arrange(grobs = list(ic80_ind_vimp_grob_lst), ncol = length(opts$importance_ind), top = "Individual Biological Variable Importance"), top = "Group Biological Variable Importance"), width = 20, height = 20, units = "cm")
}
```
...
```{r iipgroupvimp, eval = ("iip" %in% opts$outcomes) & (("marg" %in% opts$importance_grp) | ("cond" %in% opts$importance_grp)), fig.cap = biological_importance_figure_caption(ncomplete, num_obs_full, num_obs_red, "iip", grp = TRUE), fig.subcap = c("Marginal importance", "Conditional importance")}
if (("marg" %in% opts$importance_grp) & ("cond" %in% opts$importance_grp)) {
...
```{r iipindivimp, eval = ("iip" %in% opts$outcomes) & (("marg" %in% opts$importance_ind) | ("cond" %in% opts$importance_ind)), fig.cap = biological_importance_figure_caption(ncomplete, num_obs_full, num_obs_red, "iip", grp = FALSE), fig.subcap = c("Marginal importance", "Conditional importance")}
if (("marg" %in% opts$importance_ind) & ("cond" %in% opts$importance_ind)) {
...
if ("figures" %in% opts$return) {
    ggsave(filename = paste0("/home/output/iip_biol_import_ind_", postfix, ".png"), plot = grid.arrange(grobs = list(iip_ind_vimp_grob_lst), ncol = length(opts$importance_ind), top = "Individual Biological Variable Importance"), width = 20, height = 20, units = "cm")
}
```
...
Out of the `r ncomplete` sequences, `r sum(dat$dichotomous.1)` were estimated to be resistant to the `r ifelse(length(opts$nab) > 1, "combination of antibodies", "antibody")`, while `r ncomplete - sum(dat$dichotomous.1)` were estimated to be sensitive.
...
```{r plotdichot1, eval = ("sens1" %in% opts$outcomes) & (opts$cvperf), fig.cap = paste0("Cross-validated AUC for predicting sensitivity based on estimated sensitivity (n = ", ncomplete, " observations with complete sequence data).")}
...
# if saving figures was requested, save
if ("figures" %in% opts$return) {
ggsave(filename = paste0("/home/output/ic50_slplot_", postfix, ".png"), plot = plot(fit_list_out$out$sens1, main_title = "Estimated sensitivity", xlim1 = 0.4, xlim2 = 1, text_size = 9,
...
We show the biological variable importance of groups of features (defined in Table \@ref(tab:vimpGroupTab)) in predicting estimated sensitivity in Figure \@ref(fig:sens1groupvimp). Importance is defined using the difference in AUCs.
`r get_biological_importance_plot_description(opts, grp = TRUE)` `r (1-vimp_threshold)*100`\% confidence intervals and stars denoting p-values less than `r vimp_threshold` are displayed in blue.


```{r sens1groupvimp, eval = ("sens1" %in% opts$outcomes) & (("marg" %in% opts$importance_grp) | ("cond" %in% opts$importance_grp)), fig.cap = biological_importance_figure_caption(ncomplete, num_obs_full, num_obs_red, "sens1", grp = TRUE), fig.subcap = c("Marginal importance", "Conditional importance")}
if (("marg" %in% opts$importance_grp) & ("cond" %in% opts$importance_grp)) {
...
We show the biological variable importance of individual features in predicting estimated sensitivity in Figure \@ref(fig:sems1indivimp). Importance is defined using the difference in AUCs.
`r get_biological_importance_plot_description(opts, grp = FALSE)` `r (1-vimp_threshold)*100`\% confidence intervals and stars denoting p-values less than `r vimp_threshold` are displayed in blue.


```{r sens1indivimp, eval = ("sens1" %in% opts$outcomes) & (("marg" %in% opts$importance_ind) | ("cond" %in% opts$importance_ind)), fig.cap = biological_importance_figure_caption(ncomplete, num_obs_full, num_obs_red, "sens1", grp = FALSE), fig.subcap = c("Marginal importance", "Conditional importance")}
if (("marg" %in% opts$importance_ind) & ("cond" %in% opts$importance_ind)) {
...
if ("figures" %in% opts$return) {
    ggsave(filename = paste0("/home/output/sens1_biol_import_ind_", postfix, ".png"), plot = grid.arrange(grobs = list(sens1_ind_vimp_grob_lst), ncol = length(opts$importance_ind), top = "Individual Biological Variable Importance"), width = 20, height = 20, units = "cm")
}
```
...
Out of the `r ncomplete` sequences, `r sum(dat$dichotomous.2)` were estimated to be resistant to the `r ifelse(length(opts$nab) > 1, "combination of antibodies", "antibody")`, while `r ncomplete - sum(dat$dichotomous.2)` were estimated to be sensitive.
...
```{r sens2groupvimp, eval = ("sens2" %in% opts$outcomes) & (("marg" %in% opts$importance_grp) | ("cond" %in% opts$importance_grp)), fig.cap = biological_importance_figure_caption(ncomplete, num_obs_full, num_obs_red, "sens2", grp = TRUE), fig.subcap = c("Marginal importance", "Conditional importance")}
if (("marg" %in% opts$importance_grp) & ("cond" %in% opts$importance_grp)) {
...
```{r sens2indivimp, eval = ("sens2" %in% opts$outcomes) & (("marg" %in% opts$importance_ind) | ("cond" %in% opts$importance_ind)), fig.cap = biological_importance_figure_caption(ncomplete, num_obs_full, num_obs_red, "sens2", grp = FALSE), fig.subcap = c("Marginal importance", "Conditional importance")}
if (("marg" %in% opts$importance_ind) & ("cond" %in% opts$importance_ind)) {
...
if ("figures" %in% opts$return) {
    ggsave(filename = paste0("/home/output/sens2_biol_import_ind_", postfix, ".png"), plot = grid.arrange(grobs = list(sens2_ind_vimp_grob_lst), ncol = length(opts$importance_ind), top = "Individual Biological Variable Importance"), width = 20, height = 20, units = "cm")
}
```
