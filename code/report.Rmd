...
In Tables \@ref(tab:vimpgroupcond) and \@ref(tab:vimpgroupmarg), we display the groups of variables and their ranked importance for predicting each outcome. The tables are ordered by decreasing mean ranking across outcomes. For the population variable importance of each group and individual feature for predicting each outcome, please refer to the outcomes' respective sections in Section \@ref(population-variable-importance-results). For variable group definitions, please refer to Table \@ref(tab:vimpGroupTab).

```{r vimpindi, eval = run_indi_vimp}
print(paste0("In Table \@ref(tab:vimpgroupindi), we display the ranked importance of individual features for predicting each outcome. Only the features with mean rank less than ", num_pop_import, " are displayed here."))
```
