...
The estimated variable importance of the groups defined in Table \@ref(tab:vimpGroupTab) for predicting the continuous outcomes, with respect to the difference in nonparametric $R^2$, is shown in Figure \@ref(fig:groupVimpCont). The estimated variable importance of these groups for predicting the binary outcomes, with respect to the difference in nonparametric AUC, is shown in Figure \@ref(fig:groupVimpBin).

CHANGE THESE TO TABLES WITH P-VALUES

The estimated variable importance of individual features are shown in Table \@ref(tab:combinedvarimpindividualfeatures). These are features that were deemed important for prediction of at least two of the outcomes. For features associated with each outcome, please refer to the outcomes' respective sections of the report.

```{r combinedvarimpindividualfeatures}
kable(imp_overall, caption = "Variable importance of individual features, defined by being label as 'important' for prediction of at least two of the outcomes considered; these outcomes are listed in the table.")
```
...
```{r groupVimpBin, fig.cap = "Group variable importance for binary outcomes", fig.height = 10, fig.width = 20}
if (!reduce_outcomes) {
    ...
    if (!no_cv) {
        x_lab_cv <- expression(paste("Difference in CV-", AUC, sep = ""))
    ...
    grid.arrange(grobs = grob_lst, ncol = 2)
} else {
    print("Only results for IC-50 are shown, since reduce_outcomes = TRUE.")
}
```
...
```{r ic50groupvimp, fig.cap = "Group variable importance for predicting IC-50", fig.height = 10, fig.width = 20}
## continuous_outcome_vimp_plots, continuous_outcome_cv_vimp_plots are both created in preamble
ic50_grp_vimp_grob_lst <- continuous_outcome_vimp_plots[[1]]
if (!no_cv) {
    ic50_grp_vimp_grob_lst <- c(ic50_grp_vimp_grob_lst, continuous_outcome_cv_vimp_plots[[1]])
}
grid.arrange(grobs = ic50_grp_vimp_grob_lst, ncol = 2)
```


## IC-80

```{r ic80groupvimp, fig.cap = "Group variable importance for predicting IC-80", fig.height = 10, fig.width = 20}
## continuous_outcome_vimp_plots, continuous_outcome_cv_vimp_plots are both created in preamble
if (!reduce_outcomes) {
    ic80_grp_vimp_grob_lst <- continuous_outcome_vimp_plots[[2]]
    if (!no_cv) {
        ic80_grp_vimp_grob_lst <- c(ic80_grp_vimp_grob_lst, continuous_outcome_cv_vimp_plots[[2]])
    }
    grid.arrange(grobs = ic80_grp_vimp_grob_lst, ncol = 2)
} else {
    print("Only results for IC-50 are shown, since reduce_outcomes = TRUE.")
}
```
...
```{r iipgroupvimp, fig.cap = "Group variable importance for predicting IIP", fig.height = 10, fig.width = 20}
## continuous_outcome_vimp_plots, continuous_outcome_cv_vimp_plots are both created in preamble
if (!reduce_outcomes) {
    iip_grp_vimp_grob_lst <- continuous_outcome_vimp_plots[[3]]
    if (!no_cv) {
        iip_grp_vimp_grob_lst <- c(iip_grp_vimp_grob_lst, continuous_outcome_cv_vimp_plots[[3]])
    }
    grid.arrange(grobs = iip_grp_vimp_grob_lst, ncol = 2)
} else {
    print("Only results for IC-50 are shown, since reduce_outcomes = TRUE.")
}
```
...
```{r dichot1groupvimp, fig.cap = "Group variable importance for predicting estimated sensitivity", fig.height = 10, fig.width = 20}
## binary_outcome_vimp_plots, binary_outcome_cv_vimp_plots are both created in preamble
if (!reduce_outcomes) {
    dichot1_grp_vimp_grob_lst <- binary_outcome_vimp_plots[[1]]
    if (!no_cv) {
        dichot1_grp_vimp_grob_lst <- c(dichot1_grp_vimp_grob_lst, binary_outcome_cv_vimp_plots[[1]])
    }
    grid.arrange(grobs = dichot1_grp_vimp_grob_lst, ncol = 2)
} else {
    print("Only results for IC-50 are shown, since reduce_outcomes = TRUE.")
}
```
...
```{r dichot2groupvimp, fig.cap = "Group variable importance for predicting estimated sensitivity", fig.height = 10, fig.width = 20}
## binary_outcome_vimp_plots, binary_outcome_cv_vimp_plots are both created in preamble
if (!reduce_outcomes) {
    dichot2_grp_vimp_grob_lst <- binary_outcome_vimp_plots[[2]]
    if (!no_cv) {
        dichot2_grp_vimp_grob_lst <- c(dichot2_grp_vimp_grob_lst, binary_outcome_cv_vimp_plots[[2]])
    }
    grid.arrange(grobs = dichot2_grp_vimp_grob_lst, ncol = 2)
} else {
    print("Only results for IC-50 are shown, since reduce_outcomes = TRUE.")
}
```
