...
if (no_cv) {
    ...
    vimp_summary_indi <- vimp_summary_tbl %>%
        ...
        mutate(tab_outcome_name = vimp_plot_name(outcome)) %>%
        select(est, cil, ciu, p_value, tab_outcome_name, group)
    kable(vimp_summary_indi, col.names = c("Estimated variable importance", "Lower 95% CI", "Upper 95% CI", "p-value", "Outcome name", "Variable group"), caption = "Marginal variable importance of the top `r num_pop_import` individual features relative to the geographic confounders only that were deemed to have population importance statistically significantly different from zero at the `r vimp_threshold` level for the specified outcome. Importance is measured via R^2 for continuous outcomes (IC-50, IC-80, IIP) and via AUC for binary outcomes (Estimated sensitivity, Multiple Sensitivity).")
}
```
