...
This command pulls an image (a built `slapnap` container) from [DockerHub](https://hub.docker.com/). Once the image has been downloaded, we are ready to learn about how to execute `slapnap` jobs. The next section contains information on the source data used by `slapnap`. Users familiar with the CATNAP data may wish to skip directly to Section \@ref(sec:runningcontainer).
...
The CATNAP data are updated periodically. The data are downloaded into the `slapnap` container at every build. The most recent build occurred on April 29, 2021.
...
* __`outcomes`__: A semicolon-separated string of outcomes to include in the analysis. Possible values are `"ic50"` (included in default), `"ic80"`, `"iip"`, `"sens"` (included in default), `"estsens"`, and `"multsens"`. If only a single `nab` is specified, use `sens` to include a dichotomous endpoint. If multiple `nab`s are specified, use `estsens` and/or `multsens`. For detailed definitions of outcomes see Section \@ref(sec:outcomedefs).
...
* __`sens_thresh`__ A numeric value defining the neutralization threshold for defining a sensitive versus resistant pseudovirus (default = 1). The dichotomous sensitivity/resistant outcome is defined as the indicator that (estimated) IC$_{50}$ (or IC$_{80}$, if `binary_outcomes="ic80"`) is greater than or equal to `sens_thresh`.
...
* __`importance_ind`__: A semicolon-separated string indicating which individual-level variable importance measures should be computed. Possible values are none `""` (default), learner-level `"pred"`, marginal `"marg"` and conditional `"cond"`. The latter two take significant computation time to compute. See Sections \@ref(sec:biolimp) and \@ref(sec:predimp) for details on these measures.
...
There are three possible `learners` available in `slapnap`: random forests [@breiman2001], as implemented in the `R` package `ranger` [@rangerpkg] and accessed in `slapnap` by including `'rf'` in `learners`; elastic net [@zou2005] as implemented in `glmnet` [@glmnetpkg] and accessed in `slapnap` by including `'lasso'` in `learners`; and boosted trees [@friedman2001; @chen2016] as implemented in either `xgboost` [@xgboostpkg] (accessed in `slapnap` by including `'xgboost'` in `learners`) or `H2O.ai` [@h2opkg] (accessed in `slapnap` by including `'h2oboost'` in `learners`).

For each `learner`, there is a `default` choice of tuning parameters that is implemented if `cvtune="FALSE"`. If instead `cvtune="TRUE"`, then there are several choices of tuning parameters that are evaluated using `nfold` cross validation; see Table \@ref(tab:learners) for a full list of these `learners`.
...
### Intrinsic importance {#sec:biolimp}
...
### Predictive importance {#sec:predimp}
...
* a table of super learner weights (Section \@ref(sec:sldetails)) if an ensemble is used (i.e., multiple `learners` are requested);
